...
public class ClipboardSyncTests
{
    ...
    [Test]
    public async Task OnEnterRemoteScreen_ImageOnly_NoPush()
    {
        var png = MakeFakePng();
        var clipboard = new FakeClipboardSync();
        ...
        var push = relay.Sent.Where(s => s.Kind == MessageKind.ClipboardPush).ToList();
        Assert.That(push, Is.Empty);
        Assert.That(clipboard.GetImagePngCallCount, Is.Zero);
    }
    ...
    [Test]
    public async Task OnEnterRemoteScreen_ImageAndText_TextOnly()
    {
        // Plain-text policy keeps text and never reads/sends the image representation.
        var png = MakeFakePng();
        var clipboard = new FakeClipboardSync();
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(msg?.ImagePng, Is.Null);
        }

    }

    [Test]
    public async Task OnClipboardPullResponse_DropsLocalImage()
    {
        var png = MakeFakePng();
        var clipboard = new FakeClipboardSync();
        var (platform, relay, service) = CreateMasterService(clipboard);
        ...
        Assert.That(clipboard.ImagePng, Is.Null);

    }

    [Test]
    public async Task SlaveReceivesClipboardPush_DropsImage()
    {
        var png = MakeFakePng();
        var clipboard = new FakeClipboardSync();
        var slave = MakeTestableSlaveRelay(clipboard);
    ...
    [Test]
    public async Task SlaveReceivesClipboardPull_NeverCallsGetImagePng()
    {
        var png = MakeFakePng();
        var clipboard = new FakeClipboardSync();
        clipboard.SetupImage(png);
        ...
        Assert.That(clipboard.GetImagePngCallCount, Is.EqualTo(before));
    }
