    ...
    : IHostedService
{
    ...
    {
        if (!profile.AllowRemoteLock) return;
        var peerScreens = await _peerState.GetPeerScreensSnapshot();
        var hosts = peerScreens.Keys.ToArray();
        if (hosts.Length == 0) return;
    ...
    private void OnKeyEvent(KeyEvent keyEvent)
    {
        ...
        _ = _commands.Writer.TryWrite(async st =>
        {
            ...
            if (hotkeyConsumed && keyEvent.Type == KeyEventType.KeyDown && !keyEvent.IsRepeat)
            {
                ...
                {
                    if (!profile.AllowRemoteLock) { ShowOsd(st, "Remote lock disabled"); return; }
                    ...
                    // Automatic propagation has an additional screenLockPropagation gate.
                    // Every route also requires local allowRemoteLock consent.
                ...
                {
                    if (!profile.EnableFileTransfer) { ShowOsd(st, "File transfer disabled"); return; }
