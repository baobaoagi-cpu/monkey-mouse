...
// slave: also requires local syncScreensaver consent.
    ...
    IScreenSaverSync screenSaverSync, ILogger<ActivityTracker> log, Func<long>? getClock = null) : IActivityTracker
{
    ...
    {
        if (profile.SyncScreensaver) screenSaverSync.ResetIdleTimer();
    ...
    private async ValueTask BroadcastIfDue(string? excludePeer = null)
    {
        ...
        // Both roles require local consent; the receiving peer enforces its own policy too.
