...
public sealed class FileTransferService : IDisposable
{
    ...
    {
        if (!_enabled) return "File transfer disabled";
    ...
    public FileTransferService(IFileTransferDialog dialog, IDropTargetResolver dropTargetResolver, ILogger<FileTransferService> log, int watchdogTimeoutMs = 30_000, bool enableFileTransfer = false)
    {
        _enabled = enableFileTransfer;
        _dialog = dialog;
